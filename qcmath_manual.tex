...
Various CC schemes are available in qcmath, for ground-state calculations the DCD, drCCD, rCCD, lCCD, crCCD and pCCD. For excited-state calculations, the EOM-CCSD method for excited-state calculations is available. To run these calculations,  method keywords need to be specified:
\begin{itemize}
\item \keyword{"DCD"}: run a DCD calculation
\item \keyword{"drCCD"}: run a drCCD calculation
\item \keyword{"rCCD"}: run a rCCD calculation
\item \keyword{"lCCD"}: run a lCCD calculation
\item \keyword{"crCCD"}: run a crCCD calculation
